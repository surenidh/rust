...
/// Function to load holidays from embedded JSON
pub fn load_embedded_holidays() -> Result<HolidayData, Box<dyn std::error::Error>> {
    let json_str = include_str!("../holidays_2025.json");
    let data: HolidayData = serde_json::from_str(json_str)?;
    Ok(data)
}
